...
import { detectProject, ensureDir, writeFile } from "@dkit/core";
  ...
  return new Command("page")
    ...
    .action(async (name: string | undefined, opts: Record<string, unknown>) => {
      ...
      try {
        ...
        const pageTsx = renderTemplate("page", "page", { pascalName });
        writeFile(`${pageDir}/${pageFile}`, pageTsx);

        if (isAppRouter && opts.layout) {
          const layoutTsx = renderTemplate("page", "layout", { pascalName });
          writeFile(`${pageDir}/layout.tsx`, layoutTsx);
        }

        if (isAppRouter && opts.loading) {
          const loadingTsx = renderTemplate("page", "loading", { pascalName });
          writeFile(`${pageDir}/loading.tsx`, loadingTsx);
        }

        if (isAppRouter && opts.error) {
          const errorTsx = renderTemplate("page", "error", { pascalName });
        ...
        if (opts.actions) {
          ...
          const actionsTs = renderTemplate("page", "actions", {
            pascalName,
            camelName,
          });
