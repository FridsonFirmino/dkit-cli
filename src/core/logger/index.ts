...
export const logger = {
  ...
  fileCreated: (path: string) => console.log(`${colorize(icon.success, 'green')} ${colorize('Arquivo criado', 'green')} ${path}`),
  dirCreated: (path: string) => console.log(`${colorize(icon.success, 'green')} ${colorize('Diretório criado', 'green')} ${path}`),
  fileExists: (path: string) => console.log(`${colorize(icon.warn, 'yellow')} ${colorize('Arquivo já existe', 'yellow')} ${path}`),
  errorMsg: (msg: string) => console.log(`${colorize(icon.error, 'red')} ${colorize('Erro', 'red')} ${msg}`),
